...
export default async function Home({ params }) {
  ...
  const { data: mostReadArticles } = await supabase
    .from('articles')
    .select('*') // Changed to * so it fetches title_bsh
    .eq('status', 'published')
  ...
  return (
    <>
        ...
        <div className="carousel-container">
          {articles && articles.slice(0, 3).map((article, index) => (
            <div key={article.id} className={`carousel-slide${index === 0 ? ' active' : ''}`}>
              <img src={article.image_url || 'https://images.unsplash.com/photo-1620287341056-49a2f1ab2fdc?q=80&w=800&auto=format&fit=crop'} alt={getTitle(article)} className="carousel-image" />
              <div className="carousel-overlay">
                <div className="container">
                  <span className="badge badge-red">{dict.home.headline}</span>
                  <h1 className="carousel-title">{getTitle(article)}</h1>
                  <p className="carousel-excerpt">{(getDesc(article) || '').substring(0, 150) + '...'}</p>
                  <Link href={`/${lang}/article/${article.slug}`} className="btn btn-primary">{dict.home.read_more}</Link>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="flash-ticker" id="flashTicker">
        <div className="ticker-container">
          <div className="ticker-label">
            ...
            <span>{dict.home.flash_info}</span>
          </div>
          <div className="ticker-content">
            <div className="ticker-track">
              {articles && articles.map((article, i) => (
                <span key={i} className="ticker-item">🔥 {getTitle(article)}</span>
              ))}
              {articles && articles.map((article, i) => (
                <span key={`dup-${i}`} className="ticker-item">🔥 {getTitle(article)}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="article-layout-wrapper" style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 20px', display: 'flex', gap: '40px', alignItems: 'flex-start' }}>
          ...
          <div className="article-container" style={{ flex: 1, minWidth: 0 }}>
            <div className="section-header">
              ...
              <Link href={`/${lang}/faits-divers`} className="section-link">{dict.home.view_all} <i className="fas fa-arrow-right"></i></Link>
            </div>

            <div className="featured-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '30px' }}>
              {error && <p>{dict.home.error_loading}</p>}
              {articles && articles.slice(0, 6).map(article => (
                <article key={article.id} className="card article-card" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                  <div className="card-image-wrapper">
                    <img src={article.image_url || 'https://images.unsplash.com/photo-1620287341056-49a2f1ab2fdc?q=80&w=600&auto=format&fit=crop'} alt={getTitle(article)} className="card-image" loading="lazy" style={{ height: '200px', objectFit: 'cover', width: '100%' }} />
      ...
      <section style={{ padding: '0 20px 40px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        <InstagramFeed />
      </section>
